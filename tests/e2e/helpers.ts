...
type MockTopic = Record<string, unknown> & { id: number };
type MockBoard = Record<string, unknown> & { id: number };
type MockPost = Record<string, unknown>;

/**
 * Mock topic API response
 */
export const mockTopicResponse = (overrides: Partial<MockTopic> = {}): MockTopic => ({
  id: 6399262,
  title: "测试主题",
  content: "这是一个测试主题",
 ...
 */
export const mockBoardResponse = (overrides: Partial<MockBoard> = {}): MockBoard => ({
  id: 7,
  name: "技术交流",
  description: "技术交流板块",
  parentId: 0,
 ...
 */
export const mockPostResponse = (overrides: Partial<MockPost> = {}): MockPost => ({
  id: 1,
  content: "测试回复内容",
  contentType: 0,
  floor: 1,
...
export async function setupTopicPageMocks(
  ...
  await page.route(`**/topic/${topic.id}`, async (route) => {
    ...
    if (url.includes("qsh.api.cc98.top")) {
      await route.fulfill({
        status: 200,
 ...
 */
export function getPostByFloor(page: Page, floor: number): Locator {
 ...
 */
export function getLikeButton(page: Page, floor: number): Locator {
 ...
 */
export function getDislikeButton(page: Page, floor: number): Locator {
  const post = getPostByFloor(page, floor);
 ...
 */
export function getTraceButton(page: Page, floor: number): Locator {
  const post = getPostByFloor(page, floor);
 ...
 */
export function getQuoteButton(page: Page, floor: number): Locator {
  const post = getPostByFloor(page, floor);
