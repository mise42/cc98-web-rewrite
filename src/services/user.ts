...
import type { IUser, ISignIn, ITopic, IPost } from "@/types/api";

export type RecentTopic = ITopic;

export interface RecentPost extends IPost {
  topicTitle: string;
}

export interface PaginatedRecentPosts {
  data: RecentPost[];
  count: number;
}
...
export const userService = {
   ...
   * 获取当前用户最近发表的主题
   */
  async getRecentTopics(from: number = 0, size: number = 10): Promise<RecentTopic[]> {
    return apiClient.get<RecentTopic[]>(`/me/recent-topic?from=${from}&size=${size}`);
  },

  /**
   * 获取当前用户最近的回复
   */
  async getRecentPosts(from: number = 0, size: number = 10): Promise<PaginatedRecentPosts> {
    return apiClient.get<PaginatedRecentPosts>(`/me/recent-post?from=${from}&size=${size}`);
  },

  /**
   * 获取当前用户的热门回复
   */
  async getHotPosts(from: number = 0, size: number = 10): Promise<PaginatedRecentPosts> {
    return apiClient.get<PaginatedRecentPosts>(`/me/hot-post?from=${from}&size=${size}`);
  },
  ...
  async getUserRecentTopics(
    ...
    return apiClient.get<RecentTopic[]>(`/user/${userId}/recent-topic?from=${from}&size=${size}`);
  },
