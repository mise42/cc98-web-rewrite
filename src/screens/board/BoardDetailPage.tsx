...
import { ArrowLeft, MessageSquare, User, Lock } from 'lucide-react'
...
export function BoardDetailPage() {
  ...
  const boardIdParam = Array.isArray(params.boardId) ? params.boardId[0] : params.boardId
  const boardId = boardIdParam ? String(boardIdParam) : ''
  const queryClient = useQueryClient()

  // 从 URL 读取查询参数
  const searchParams = useSearchParams()
  const urlPage = Number(searchParams.get('page') ?? '1')
  const urlMode = searchParams.get('mode') === 'infinite' ? 'infinite' : 'pagination'
  ...
  const updateURL = (page: number, mode: 'pagination' | 'infinite') => {
    router.push(`/board/${boardId}?page=${page}&mode=${mode}`)
  }
  ...
  // 分页模式使用过滤后的帖子
  const filteredPagedTopics = useMemo(() => filterTopics(pagedTopics), [pagedTopics, filterTopics])
  // 无限滚动模式使用过滤后的帖子
  const filteredAllTopics = useMemo(() => filterTopics(allTopics), [allTopics, filterTopics])
