...
export function TopicDetailPage() {
  ...
  const numericTopicId = parseInt(String(topicIdParam), 10)
  ...
  const updateURL = (page: number, mode: 'pagination' | 'infinite') => {
    ...
    router.push(`/topic/${topicIdParam}?page=${page}&mode=${mode}`)
  }
  ...
  const {
    ...
    retry: (failureCount, error) => {
      if (error instanceof Error && getErrorStatus(error) === 401) {
        return false
      }
  ...
  // 无限滚动模式：初始加载
  const { isLoading: initialPostsLoading, error: initialPostsError } = useQuery<IPost[]>({
