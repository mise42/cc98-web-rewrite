...
export class UbbTextSegment extends UbbSegment {
  get type(): UbbSegmentType {
    return UbbSegmentType.Text
  }
...
export class UbbTagSegment extends UbbSegment {
  ...
  get tagData(): UbbTagData {
    return this._tagData
  }
  get subSegments(): UbbSegment[] {
  ...
  private static forceClose(segment: UbbSegment, newParent: UbbTagSegment): void {
    ...
    } else if (segment.type === UbbSegmentType.EmptyTag) {
      //不必显式配对的标签也无需关闭
      newParent._subSegments.push(segment.clone(newParent))
    } else {
      // 已经关闭的标签也无需关闭
  ...
  getContentText() {
    ...
    for (const subItem of this._subSegments) {
      ...
      } else if (subItem.type === UbbSegmentType.EmptyTag) {
        const tagName = (subItem as UbbEmptyTagSegment).tagName
        const isExplicitlyPaired = (subItem as UbbEmptyTagSegment).isExplicitlyPaired
        if (isExplicitlyPaired) {
          subContents.push(`[${tagName}][/${tagName}]`)
        } else {
          subContents.push(`[${tagName}]`)
        }
      } else {
 ...
 * 表示不必显式配对的 UBB 标签片段。
 */
export class UbbEmptyTagSegment extends UbbTagSegment {
  get type(): UbbSegmentType {
    return UbbSegmentType.EmptyTag
  }
  ...
  /**
   * 标签是否显式地配对，如[ac01][/ac01]。
   */
  private _isExplicitlyPaired: boolean

  /**
   * 获取一个值，指示标签是否显式地配对。
   */
  get isExplicitlyPaired() {
    return this._isExplicitlyPaired
  }

  /**
   * 创建一个新的 UbbEmptyTagSegment 对象。
   * @param tagName 标签的名称，如ac01。
   * @param isExplicitlyPaired 标签是否显式地配对，如[ac01][/ac01]。
   */
  constructor(
    tagData: UbbTagData,
    tagName: string,
    isExplicitlyPaired: boolean,
    parent: UbbTagSegment
  ) {
    super(tagData, parent)
    this._tagName = tagName
    this._isExplicitlyPaired = isExplicitlyPaired
  }

  clone(newParent: UbbTagSegment): UbbSegment {
    return new UbbEmptyTagSegment(
      this.getTagData,
      this._tagName,
      this._isExplicitlyPaired,
      newParent
    )
  }
}
...
export class UbbTagData {
  ...
  constructor(originalString: string, parameters: UbbTagParameter[]) {
    if (!parameters) {
      ...
      function scanToken(lastTokenType: TokenType): Token {
        ...
        function scanQuoted(quoteType: string) {
          ...
          if (endMarkLocation < 0) {
            console.error(
              'UBB: 解析标签字符串 %s 时无法找到位置 %d 处 %s 对应的结束字符串。',
              tokenString,
              index,
              quoteMark
            )
...
export abstract class UbbTagHandler {
  ...
  protected static renderTagAsString(tagData: UbbTagData, content: ReactNode): ReactNode {
    return [tagData.startTagString, content, tagData.endTagString]
  }
}
...
export abstract class TextTagHandler extends UbbTagHandler {
   ...
   */
  protected abstract execCore(
    innerContent: string,
    tagData: UbbTagData,
    context: UbbCodeContext
  ): ReactNode
}
    ...
    while (true) {
      ...
      if (endTagMatch) {
        ...
        try {
          ...
          if (!handler) {
            console.warn(
              'UBB: 标签字符串 %s 中给定的标签名 %s 没有有效的处理程序，将被视为一般文字',
              tagString,
              tagData.tagName
            )
          ...
          switch (tagMode) {
            ...
            case UbbTagMode.Text:
              ...
              if (endTagMatch === -1) {
                console.warn(
                  'UBB: 找不到标签字符串 %s 对应的结束标签，该内容将被视为一般文字。',
            ...
            case UbbTagMode.Empty:
              // 如果紧跟着结束标签，即显式地配对标签，如[ac01][/ac01]
              if (content.startsWith(tagData.endTagString, tagMatchRegExp.lastIndex)) {
                const tagName = tagData.originalString
                const isExplicitlyPaired = true
                const newEmptyTag = new UbbEmptyTagSegment(
                  tagData,
                  tagName,
                  isExplicitlyPaired,
                  parent
                ...
                const tagName = tagData.originalString
                const isExplicitlyPaired = false
                const newEmptyTag = new UbbEmptyTagSegment(
                  tagData,
                  tagName,
                  isExplicitlyPaired,
