...
export class ImageTagHandler extends Ubb.TextTagHandler {
  ...
  execCore(content: string, tagData: Ubb.UbbTagData, context: Ubb.UbbCodeContext): React.ReactNode {
    ...
    if (context.options.compatibility === Ubb.UbbCompatiblityMode.EnforceMorden) {
      if (isShowedValue === 1) {
        return (
          <Image imageUri={imageUri} title={title} isShowed={false} allowToolbox={allowToolbox} />
        )
      } else {
        return (
          <figure>
            <Image imageUri={imageUri} title={title} isShowed={true} allowToolbox={allowToolbox} />
...
> {
  ...
  render() {
    ...
    if (this.state.isShowed) {
      //允许显示图片工具箱。
      if (this.props.allowToolbox && !this.state.neverShowToolbox) {
        let toolbox = null
        //鼠标移至图片上，显示工具箱。
        if (this.state.isToolboxOn) {
          toolbox = (
            <div className="ubb-image-toolbox">
              ...
              <button
                onClick={() => {
                  this.setState({ neverShowToolbox: true })
                }}
              >
