...
export default class BiliTextHandler extends Ubb.UbbTextHandler {
   ...
   */
  private _pattern =
    /(https?:\/\/)?www.bilibili.com\/video\/[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]/
  ...
  exec(match: RegExpMatchArray, context: Ubb.UbbCodeContext): React.ReactNode {
    ...
    if (!context.options.allowMediaContent) {
      return context.options.autoDetectUrl ? (
        <a href={content} target="_blank" className="urlStyle">
          {content}
        </a>
      ) : (
        content
    ...
    if (isAv) {
      ...
      const src = `https://player.bilibili.com/player.html?aid=${av}&page=${page}&autoplay=0&poster=1`
      return (
        <iframe
          {...props}
          src={src}
      ...
      const src = `https://player.bilibili.com/player.html?bvid=${BV}&page=${page}&autoplay=0&poster=1`
      return (
        <iframe
          {...props}
          src={src}
