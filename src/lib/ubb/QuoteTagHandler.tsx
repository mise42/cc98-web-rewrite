  ...
  exec(tagSegment: Ubb.UbbTagSegment, context: Ubb.UbbCodeContext) {
    ...
    const isPureQuote = (segments: Ubb.UbbSegment[]): boolean => {
      ...
      return (
        ...
        ((segments[0] as Ubb.UbbTagSegment).subSegments[0] as Ubb.UbbTextSegment).text.indexOf(
          '以下是引用'
        ) === 0 &&
        segments
          ...
          .every(
            item =>
              item.type === Ubb.UbbSegmentType.Text &&
    ...
    if (queue.length > 1) {
      ...
      for (let i = 1; i < queue.length; i++) {
        if (isPureQuote(queue[i].subSegments)) {
          const info = (queue[i].subSegments.shift() as Ubb.UbbTagSegment).clone(queue[i - 1])
