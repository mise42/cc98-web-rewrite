...
export default abstract class MessageBarTagHandlerBase extends Ubb.RecursiveTagHandler {
  ...
  execCore(
    ...
    if (messageIndex >= this.messageSettings.length || messageIndex < 0) {
      throw new Error(
        `标签 ${tagData.tagName} 配置的消息参数 ${messageIndex} 无效，该标签最多支持 ${this.messageSettings.length} 种消息模式。`
      )
    }
    ...
    return (
      <MessageBar
        message={actualMessageSetting.message}
        messageType={actualMessageSetting.messageType}
      />
    )
  }
}
