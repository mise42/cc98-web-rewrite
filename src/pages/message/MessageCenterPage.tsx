...
import { messageService, type INotification, type IUnreadMessageCount } from "@/services/message";
  ...
  if (error) {
    return (
      <ErrorState
        ...
        retry={() =>
          queryClient.invalidateQueries({ queryKey: ["message", "notifications", queryKey] })
        }
      />
    );
  }
            ...
            <div className="divide-y divide-border">
              {notifications.map((notification) => (
                <NotificationItem
                  ...
                  topicTitle={
                    notification.topicId ? topicTitleMap[notification.topicId] : undefined
                  }
                />
              ))}
            </div>
    ...
    <div className={`px-4 py-3 ${notification.isRead ? "opacity-80" : ""}`}>
      <div className="min-w-0">
        ...
        <div className="mt-2 text-xs text-muted-foreground">
          ...
          {notification.postBasicInfo?.floor && (
            <span className="ml-2">#{notification.postBasicInfo.floor} 楼</span>
          )}
        </div>
      </div>
    </div>
...
function PrivateMessagePanel() {
  ...
  const { data: userMap = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      return users.reduce<Record<number, Awaited<ReturnType<typeof userService.getUser>>>>(
        (map, item) => {
          if (item) {
            map[item.id] = item;
          }
  ...
  const activeUserId =
    selectedUserId && contactIds.includes(selectedUserId)
      ? selectedUserId
      : (contactIds[0] ?? null);
  ...
  const {
    ...
    queryKey: ["message", "conversation", activeUserId],
    queryFn: () => messageService.getMessageContent(activeUserId!, 0, 20),
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      const targetUser = userMap[activeUserId];
      if (!targetUser?.name) {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-md p-3 text-left transition-colors ${
                      activeUserId === contact.userId ? "bg-primary/10" : "hover:bg-muted"
                    }`}
                  >
