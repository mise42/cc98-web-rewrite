...
const ubbActions: ToolbarAction[] = [
  { key: 'bold', label: '加粗', icon: Bold, apply: selected => `[b]${selected || '加粗文字'}[/b]` },
  {
    key: 'italic',
    label: '斜体',
    icon: Italic,
...
const markdownActions: ToolbarAction[] = [
  { key: 'heading', label: '标题', icon: Heading1, apply: selected => `## ${selected || '标题'}` },
  { key: 'bold', label: '加粗', icon: Bold, apply: selected => `**${selected || '加粗文字'}**` },
  { key: 'italic', label: '斜体', icon: Italic, apply: selected => `*${selected || '斜体文字'}*` },
  {
    key: 'strike',
    label: '删除线',
    icon: Strikethrough,
...
export function FormatToolbar({ mode, value, onChange, textareaRef }: FormatToolbarProps) {
  ...
  const [recentEmojiTokens, setRecentEmojiTokens] = useState<string[]>([])
  ...
  const recentUbbEmojis = useMemo(
    ...
    [allUbbEmojis, recentEmojiTokens, ubbEmojiLookup]
  )
  ...
  useEffect(() => {
    try {
      ...
      const validTokens = parsed
        ...
        .filter(token => ubbEmojiLookup.has(token))
        .slice(0, RECENT_EMOJI_LIMIT)

      setRecentEmojiTokens(validTokens)
    } catch {
  ...
  const applyAction = (action: ToolbarAction) => {
    const inserted = action.apply(currentSelection)
    insertText(inserted)
  }
  ...
  const insertLink = () => {
    ...
    const text = linkText.trim() || currentSelection || '链接'
    const inserted =
      mode === 'ubb'
        ? text === linkUrl.trim()
